var app = new Vue({
    ...
    methods: {
        ...
        send: function () {
            if (this.validate(this.dataBits) === true){
                var encodedMessage = this.encode(this.dataBits, 
                    this.numberOfDataBits, 
                    this.parityBitOption);

                // display info
                this.parityBit = 'Parity bit option: ' + this.parityBitOption;
                this.dataBitsNumber = 'Number of data bits: ' + this.numberOfDataBits;
                this.sentMessage = 'Message sent to server: ' + encodedMessage;

                return axios.put("http://localhost:3000/message", 
                    {bits: encodedMessage, numberOfDataBits: this.numberOfDataBits, parityBitOption: this.parityBitOption})
        ...
        encode: function(bits, numberOfDataBits, parityBitOption){
            if (numberOfDataBits == 4){
